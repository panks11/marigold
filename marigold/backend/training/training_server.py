...
from marigold.backend.training.connection_client import ConnectionClient as TrainingConnection
from marigold.backend.training.types import TrainingRunInput
from marigold.utils import default_logger


class TrainingServer(MarigoldBase):
    """Marigold Training Server

    The Marigold Training Server provides a unified interface for starting training runs. It is used by the gateway
    ...
    code::
        ...
        $ python -m gunicorn -w 1 -b localhost:8081 -k uvicorn.workers.UvicornWorker \
            marigold.backend.training.training_server:app

    """

    logger = default_logger(
        name="marigold.backend.training.training_server.TrainingServer",

...
from marigold.backend.deployment.connection_client import ConnectionClient as DeploymentConnection
from marigold.backend.deployment.types import ClassifyIDInput, ClassifyImageInput, LoadModelInput
from marigold.data import MNIST
from marigold.modules import Registry
from marigold.utils import ascii_to_pil, default_logger, pil_to_ascii, pil_to_ndarray, tensor_to_pil


class DeploymentServer(MarigoldBase):
    """Marigold Deployment Server

    The Marigold Deployment Server provides a unified interface for deploying models. It is used by the gateway server
    to deploy models.
    ...
    code::
        ...
        $ python -m gunicorn -w 1 -b localhost:8083 -k uvicorn.workers.UvicornWorker \
            marigold.backend.deployment.deployment_server:app

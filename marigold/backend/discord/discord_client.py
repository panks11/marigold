...
from marigold import Config, MarigoldBase
from marigold.backend.gateway import GatewayServer
from marigold.modules import Registry
from marigold.utils import bytes_to_pil, default_logger, ifnone


class DiscordClient(MarigoldBase):
    """Marigold Discord Client

    The Marigold Discord Client provides a listener for the discord bot associated with the API key listed in the
    config.ini file. It is used by the backend to communicate with the Marigold discord server, although it could also
    be used to power a personal discord bot by changing the API key.

    Args:
        gateway_host: The URI of the Marigold gateway server. If not given, defaults to the URI specified in the
            config file.
        description: The description of the discord bot.
        command_prefixes: The command prefixes for the discord bot. Defaults to ['>', 'marigold '].
    ...
    def discord_bot(self):
        ...
        async def on_message(message):
            ...
            elif "marigold" in message.content.lower():

...
from marigold import MarigoldBase
from marigold.backend.deployment import DeploymentServer
from marigold.backend.gateway.connection_client import ConnectionClient as GatewayConnection
from marigold.backend.gateway.types import (
...
from marigold.backend.training import TrainingServer
from marigold.data import MNIST
from marigold.modules import GPT, Registry
from marigold.types import Message
from marigold.utils import ascii_to_pil, default_logger, ifnone, pil_to_ascii


class GatewayServer(MarigoldBase):
    """Marigold Gateway Server

    The Marigold Gateway Server provides a unified interface for communicating with the Marigold backend. It is used
    ...
    code::
        ...
        $ python -m gunicorn -w 1 -b localhost:8081 -k uvicorn.workers.UvicornWorker \
            marigold.backend.gateway.server:app
    ...
    def app(self):
        ...
        def chat(payload: ChatInput):
            ...
            if self.gpt is not None:
                ...
                response = Message(sender="marigold", text="Sorry, I don't know how to chat yet.")
        ...
        def debug(payload: DebugInput):
            ...
            instructions = (
                ...
                f"marigold project. You will talk with the marigold application developers and answer their "

...
Awesome marigold created by panks11

# Installation

```commandline
$ git clone git@github.com:panks11/marigold.git && cd marigold
```
...
The marigold package is an end-to-end machine learning framework for training and deploying models. It is meant 
to help kickstart new projects, containing all the structural components necessary to train, manage and deploy models. 
Marigold has been created in a modular design, so if a particular component is not needed, it can simply be deleted.

At a high level, the marigold package covers the following components of the ML lifecycle:
...
- **Packaging**, including both python (`pip install marigold`) and docker (`docker compose up`) functionality; 
...
The Marigold package has been tested on Ubuntu 22.04 with Python 3.10.12.

## Package Components

The marigold package is organized into the following components:
...
- **[docker](docker)**. Contains Dockerfiles for building the marigold Docker image. Refer to the [Docker](./docker) 
    module for more details;
- **[marigold](marigold)**: The main package, containing all the core functionality of the project;
  - **[marigold/backend](marigold/backend)**: Contains modules for running each of the discord, gateway and 
      training servers;;
  - **[marigold/configs](marigold/configs)**: Contains the (Hydra) yaml files for model training;
  - **[marigold/core](marigold/core)**: Contains core marigold packages, including:
    - **[Config](marigold/core/config.py)**. This class provides access to the associated 
      [config.ini](marigold/core/config.ini) file, used for package-level configurations;
    - **[MarigoldBase](marigold/core/base.py)**. This class serves as a helper base class. It provides unified 
      logging functionality, among other features.
  - **[marigold/data](marigold/data)**. Contains modules for individual `pl.LightningDataModule` datasets;
  - **[marigold/models](marigold/models)**. Contains modules for individual `torch.nn.Module` models; also includes 
      a torch Lightning wrapper class for `torch.nn.Modules`, which can be used to wrap models during training;
  - **[marigold/modules](marigold/modules)**: Modules used within the package, including model Registry and GPT 
      classes;
  - **[marigold/utils](marigold/utils)**: Utility functions used throughout the package. Notable utilities include:
    - **[conversions](marigold/utils/conversions.py)**. Provides methods for converting and serializing PIL images, 
      used to pass images between servers;
    - **[logging](marigold/utils/logging.py)**. Provides a `default_logger` method, may be used to customize the 
      logger provided by the BaseMarigold class;
  - **[marigold/scripts](marigold/scripts)**: Standalone scripts built on top of the marigold package; the project 
      starts with a training script that demonstrates how to set up a training pipeline to store models to the model 
      registry;
- **[tests](tests)**: Contains unit tests for the marigold package. Refer to the [Unit Tests](#Unit-Tests) section for 
    more details;
...
tests/marigold/core/test_config.py ...                                 [ 23%]
...
tests/marigold/utils/test_timer_collection.py .                        [100%]

---------- coverage: platform linux, python 3.11.6-final-0 -----------
Name                                               Stmts   Miss  Cover   Missing
--------------------------------------------------------------------------------
marigold/__init__.py                                 3      0   100%
...
marigold/utils/timer_collection.py                  28      0   100%
--------------------------------------------------------------------------------
TOTAL                                               1134    854    25%


=========================== short test summary info ============================
SKIPPED [1] tests/marigold/core/test_registry.py:12: No runs found in MLflow.
...
`docs/_build/simplepdf/Marigold.pdf` respectively.
...
* Building wheel...
Successfully built marigold-0.1.0.tar.gz and marigold-0.1.0-py3-none-any.whl
```
...
from [GithubActions](https://github.com/panks11/marigold/actions). 

# End-to-End Demo

The end-to-end application demo allows training and deploying models through a private discord server. It is meant to 
showcase the full functionality of the marigold package, and provide a starting point for deploying your own models.
...
Finally, add your discord bot token to your [config.ini](marigold/core/config.ini) file under API_KEYS/DISCORD. You may 
...
[config.ini](marigold/core/config.ini) file under API_KEYS/OPENAI.
...
[config.ini](marigold/core/config.ini) file to match, or pass them in as command line arguments to each other server. 
  - The default configs assume that output data (MLFlow registry, hydra training runs, tensorboard logs, standard 
debug logs, etc.) will be stored in `${HOME}/marigold` and, for unit tests, that the project has been installed in 
`${HOME}/projects/marigold`. If you wish to change these locations, you will need to update the same 
[config.ini](marigold/core/config.ini) file accordingly. To create the default directories automatically, you may run 
...
in a completely separate environment, and configure each training job to get a separate GPU. 
  - If, at any point, you get an error saying `marigold` cannot be found, remember to add the marigold path to your 
PYTHONPATH variable. E.g. 

```commandline
PYTHONPATH=${HOME}/projects/marigold [... continue command]
```
...
```commandline
mlflow server --backend-store-uri ${HOME}/marigold/mlflow --port 8080
```
...
```commandline
python -m gunicorn -w 1 -b localhost:8081 -k uvicorn.workers.UvicornWorker "marigold.backend.gateway.gateway_server:app()"
```
...
```commandline
python -m gunicorn -w 4 -b localhost:8082 -k uvicorn.workers.UvicornWorker "marigold.backend.training.training_server:app()"
```
...
```commandline
python -m gunicorn -w 1 -b localhost:8083 -k uvicorn.workers.UvicornWorker "marigold.backend.deployment.deployment_server:app()"
```
...
```commandline
python marigold/backend/discord/discord_client.py
```
